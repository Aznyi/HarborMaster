...
| R30 | **A failed MANUAL recreation leaves two containers on the host and does not restore service by itself** | **Medium** | Deliberate for an update a person asked for: an automatic undo is another unattended mutation performed at exactly the moment HarborMaster has demonstrated its model of the host is wrong, and the operator is present. An UNATTENDED update is the case where nobody is present, and automation may undo its own failure where the governing policy permits -- see R48 | The original is never removed before the replacement is fully proved; the failed replacement is stopped and renamed off the production name; the record carries a `recovery` plan naming both containers by name and id with the exact commands; the summary counts these separately as `needsAttention`, and retention never prunes them |
