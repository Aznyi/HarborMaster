-- 0030_execution_snapshot_changed: accept the snapshotChanged refusal.
--
-- # What this value means, and why it needed to exist
--
-- Phase 17 makes HarborMaster capture a container's configuration snapshot by
-- itself, so an operator does not have to snapshot every container by hand
-- before automatic updates can work. That raised a question the codebase had
-- not had to answer before: what happens when the snapshot taken immediately
-- before a recreation is NOT the snapshot the change plan was assessed against.
--
-- The answer is that the plan is stale. Its risk assessment was computed against
-- a baseline that described the container as it was, and the container has since
-- been reconfigured. A change plan is immutable, so the new baseline cannot be
-- read into it -- the assessment an operator reviewed would stop being the
-- assessment that authorised the change. The recreation is refused and the next
-- planner pass produces a plan against the new evidence.
--
-- # Why not an existing refusal
--
-- Each of the three near neighbours says something that is not true here:
--
--     snapshotMissing     a snapshot exists
--     restoreReadiness    the snapshot is usable; it just describes a different
--                         configuration
--     containerChanged    the container still runs the image the plan assessed,
--                         which is the only thing that check looks at
--
-- Collapsing this into any of them would leave an operator reading a reason that
-- points at the wrong remedy.
--
-- # Why a table rebuild
--
-- SQLite cannot alter a CHECK in place. Same procedure as 0014, 0017, 0021,
-- 0026, 0027, 0028 and 0029: copy, drop, rename, recreate the indexes.
--
-- The definition below is 0028's, which is the LIVE schema for this table --
-- nothing between 0028 and here altered `executions`. The only difference is the
-- one value added to the refusal CHECK.
--
